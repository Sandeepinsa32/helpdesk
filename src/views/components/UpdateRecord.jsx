  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      bookingType: Yup.string()
        .oneOf(
          ["new", "exchange", "refund", "void", "addon"],
      ...
      childPrice: Yup.number("input must consist if number").when(
        ["childCount"],
        (childCount, schema) => {
          return childCount > 0
            ? schema
                .required("this field required ")
                .positive("Price Should be Greater then 0 ")
                .integer()
            : schema;
        }
      ),
      adultPrice: Yup.number("input must consist if number").when(
        ["adultCount"],
        (adultCount, schema) => {
          return adultCount > 0
            ? schema
      ...
      elderPrice: Yup.number("input must consist if number").when(
        ["elderCount"],
        (elderCount, schema) => {
          return elderCount > 0
            ? schema
      ...
      card: Yup.array().of(
        Yup.object().shape({
          ...
          cvv: Yup.string()
            .min(3, "cvv must have atleast3  characters")
            .max(4, "cvv must have atleast 4 characters"),
    ...
    onSubmit: () => {
      ...
      const { alternateEmail, alternatePhone, pnrNo, airlineLocator } =
        formik.values;
                      ...
                      <Grid container spacing={3} key={i}>
                        ...
                        <Grid item xs={4} md={2}>
                          <TextField
                            ...
                            fullWidth
                            disabled={true}
                            value={alreadyPresentCard[i].cardHolderNumber}
                          />
                        </Grid>
                        {/* CardNumber Field */}
                        <Grid item xs={4} md={3}>
                        ...
                        <Grid item xs={4} md={2}>
                          <TextField
                            ...
                            fullWidth
                            disabled={true}
                            value={alreadyPresentCard[i].cvv}
                          />
                        </Grid>
                        {/* expiry date field */}
                        <Grid item xs={4} md={3}>
                      ...
                      <TextField
                        ...
                        error={Boolean(
                          formik.submitCount > 0 &&
                            err.length > 0 &&
                            err[i] !== (undefined && null)
                            ? err[i]["cardHolderName"]
                            : null
                        )}
                        helperText={
                          formik.submitCount > 0 &&
