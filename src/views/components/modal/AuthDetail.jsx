...
function AuthDetail({ Ticketid, TicketData, onClose }) {
  const { authorizedIps, firstName, lastName, email, bookingId, comments } =
    TicketData;
  ...
  return (
    <>
      {isLoading || chargeRequests.length == 0 ? (
        <Box
          sx={{
            display: "flex",
            minHeight: "70vh",
        ...
        <Box ref={componentRef}>
          ...
          <CardActions
            sx={{ display: "flex", justifyContent: "end", padding: "20px  " }}
          >
            <ReactToPrint
